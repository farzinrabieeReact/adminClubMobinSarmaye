...
import { dateMiladi } from "../../../common/method/date/index";
import { useDispatch } from "react-redux";
import { actionTypes } from "../../../../redux/education/education_newCourse_insert/index";
...
const InsertEducation = () => {
  ...
  const handelSubmit = () => {
    ...
    if( !state.start_date ||  !state.end_date || !state.registration_start_date || !state.registration_end_date){
      dispatch({
        type: actionTypesNotif.warning,
        textAlert: "لطفا فیلد های مورد نظر را پر نمایید"
    });
      return null
    }

    let startDate = convertDigitToEnglish(
      state.start_date.format("jYYYY/jMM/jDD")
    );
    let endDate = convertDigitToEnglish(state.end_date.format("jYYYY/jMM/jDD"));

    let registerStartDate = convertDigitToEnglish(`
    
    ${state.registration_start_date.format("jYYYY/jMM/jDD")}
    `);

    let registerEndtDate = convertDigitToEnglish(`
    
    ${state.registration_end_date.format("jYYYY/jMM/jDD")}
    `);

    state.start_date = startDate;
    state.end_date = endDate;
    state.registration_start_date =
      dateMiladi(registerStartDate) + " 00:00:00.000000";

    state.registration_end_date =
      dateMiladi(registerEndtDate) + " 23:59:59.000000";


    dispatch({
      type: actionTypes.educationInsertNewCourseAsync,
      payload: state
    });
        ...
        <div className={classes["root"]}>
          ...
          <Box width="23%">
            <DatePicker
              ...
              setValue={data => handelCHnage(data, "registration_start_date")}
            />
          </Box>
          ...
          <Box width="23%">
            <DatePicker
              ...
              setValue={data => handelCHnage(data, "registration_end_date")}
            />
          </Box>
