...
export default function Index({}) {
    ...
    const [valueInsert, setValueInsert] = useState({ competition_title: "", required_bonus: "", reward_bonus: "", start_date: null, participation_deadline: null, question: "", correct_answer_description: "", updatable: "TRUE", time_start: null })


    const dispatch = useDispatch();
    const submitInsertCompetition = (data) => {
    //   dispatch(compatition_actions_insert(data));
    compatition_actions_insert(data)
    .then((result) => {
...
// console.log("dateeeeeeeeeee",valueInsert.start_date.format("YYYY/MM/DD"))
    /////////////////////////step3/////////////////////////
    ...
    const handleComplete = () => {
        if (completedSteps() === totalSteps() - 1) {
            ...
            if(stateStep3){
                let isok = stateStep3.some(e => e.isTrust === true)
                if(!isok){
                    alert("در مرحله 3 یک گزینه را انتخاب نمایید")
                    return 
                }
            }
            ...
            let result = {
                ...
                start_date: `${convertDigitToEnglish(valueInsert.start_date.format("YYYY/MM/DD"))} ${convertDigitToEnglish(valueInsert.time_start.format("hh:mm:ss.000000"))}`,
                participation_deadline: `${convertDigitToEnglish(valueInsert.participation_deadline.format("YYYY/MM/DD"))} 23:59:00.000000`,
