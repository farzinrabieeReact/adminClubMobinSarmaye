...
import { deactivate_memberSubscriptions_dispatch } from "../../../../../redux/signalHafez/memberSubscriptions/deactive_memberSubscriptions";
import { active_memberSubscriptions_dispatch } from "../../../../../redux/signalHafez/memberSubscriptions/active_memberSubscriptions";
...
export default function Index({
  ...
  //     update_signal_document_dispatch(_data)
  //         .then(result => {
  ...
  const handleOkAlertDeactive = (id: any) => {
    ...
    deactivate_memberSubscriptions_dispatch(_data)
      .then((result: any) => {
        let isOk = handleNotificationAlertTryUpdate(result);
        if (!isOk) {
          return;
  ...
  const handleOkAlertActive = (id: any) => {
    ...
    active_memberSubscriptions_dispatch(_data)
      .then((result: any) => {
        let isOk = handleNotificationAlertTryUpdate(result);
        if (!isOk) {
          return;
    ...
    <>
      <TableRow key={index}>
        ...
        {head
          .filter(
            (column: any, index: any) =>
              index !== 0 && index !== head.length - 1
          )
          .map((column: any, index: any) => {
            let value = item.body[column.title];

            return (
              <TableCell align="center" style={{ maxWidth: 200 }} key={index}>
        ...
        >
          ...
          {item.body.status === "REJECTED" && (
            <button
              className={`btnsGreen ${
                item.body.is_active === "FALSE" ? "disabledItems" : ""
              }`}
              onClick={() => setAlertActive([true, item.id])}
            >
              فعال کردن
        ...
        <AlertDialogSlide
          ...
          data={{
            ...
            description: loading ? (
              ...
              "آیا از غیر فعال کردن این آیتم اطمینان دارید؟"
            )
          }}
        />
      )}
        ...
        <AlertDialogSlide
          ...
          data={{
            ...
            description: loading ? (
              ...
              "آیا از  فعال کردن این آیتم اطمینان دارید؟"
            )
          }}
        />
      )}
